...
const FooterNav: FC<Props> = ({ lightTheme = true, prev, next, className }): JSX.Element => {
  ...
  return (
    <nav className={`bottom-0 flex justify-center w-full space-x-4 pb-8 ${className}`}>
      {prev ? (
        <button
          type="button"
          ...
          } inline-flex items-center px-6 py-2 h-12 flex-shrink-0 text-lg font-medium border rounded-md focus:outline-none`}
        >
          ย้อนกลับ
        </button>
      ) : (
      ...
      {next ? (
        <button
          ...
          } inline-flex items-center px-6 py-2 h-12 flex-shrink-0 text-lg font-medium border shadow-md rounded-md focus:outline-none`}
        >
          หน้าถัดไป
        </button>
      ) : (
